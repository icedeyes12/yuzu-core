...
class YuzuCompanionAgent:
    def __init__(self):
        ...
        self.session_state = {
            "streaming": self.profile.get("providers_config", {}).get(
                "streaming_enabled", False
            ),
            "system_prompt": None,
            "current_provider": self.profile.get("providers_config", {}).get(
                "preferred_provider", "ollama"
            ),
            "current_model": self.profile.get("providers_config", {}).get(
    ...
    def profile(self):
        ...
        if (
            ...
            or current_time - self._last_profile_update > self.CACHE_TTL
        ):
    ...
    def session(self):
        ...
        if (
            ...
            or current_time - self._last_session_update > self.CACHE_TTL
        ):
    ...
    def load_config(self):
        config_path = os.path.join(
            os.path.expanduser("~"), ".yuzu_companion_terminal.json"
        )
        default_config = {
    ...
    def handle_model_command(self, args):
        if not args:
            console.print(
                "Usage: [cyan]/model <provider>/<model>[/] OR [cyan]/model <model_name>[/]"
            )
            return
    ...
    def show_available_models(self):
        ...
        for provider, models in all_models.items():
            current_marker = (
                " *" if provider == self.session_state["current_provider"] else ""
            )
            models_list = []
            for model in models[:8]:
                model_marker = (
                    " *" if model == self.session_state["current_model"] else ""
                )
                vision_marker = (
                    " [V]" if multimodal_tools.is_vision_model(model, provider) else ""
                )
                models_list.append(f"{model}{model_marker}{vision_marker}")

            table.add_row(f"{provider}{current_marker}", "\n".join(models_list))
    ...
    def show_available_providers(self):
        ...
        for provider in providers:
            current_marker = (
                " *CURRENT"
                if provider == self.session_state["current_provider"]
                else ""
            )
            ...
            status = (
                "Connected"
                if provider_obj and provider_obj.test_connection()
                else "Failed"
            )
    ...
    def show_status_info(self):
        ...
        status_table.add_row(
            "Session", f"{active_session.get('name')} (ID: {active_session.get('id')})"
        )
        status_table.add_row("Messages", str(active_session.get("message_count", 0)))
        status_table.add_row(
            "AI Provider",
            f"{self.session_state['current_provider']}/{self.session_state['current_model']}",
        )
        status_table.add_row(
            "Streaming", "ON" if self.session_state["streaming"] else "OFF"
        )
        status_table.add_row(
            "System Prompt", self.session_state["system_prompt"] or "Not set"
        )
        status_table.add_row(
            "Session Context", f"{len(session_memory.get('session_context', ''))} chars"
        )
        status_table.add_row(
            "Vision",
    ...
    def show_capabilities(self):
        ...
        table.add_row(
            ...
            if self.vision_capabilities["has_vision"]
            else "None",
        )
    ...
    def update_session_state(self):
        ...
        self.session_state.update(
            {
                "current_provider": providers_config.get(
                    "preferred_provider", "ollama"
                ),
                "current_model": providers_config.get(
                    "preferred_model", "glm-4.6:cloud"
                ),
    ...
    def display_normal_response_panel(self, response: str, response_time: float):
        ...
        if not response.strip():
            console.print(
                Panel("(Empty response)", title=title, border_style="magenta")
            )
            return
        ...
        if code_blocks:
            ...
            renderable = (
                Markdown(response) if any(c in response for c in "#*`>") else response
            )
    ...
    def display_streaming_response_panel(self, response_generator):
        ...
        streaming_panel = Panel(
            current_display, title=title, title_align="left", border_style="magenta"
        )
    ...
    def _initialize_prompt_session(self):
        return PromptSession(
            ...
            completer=WordCompleter(
                [cmd for category in self.commands.values() for cmd in category],
                ignore_case=True,
            ),
        )
    ...
    def _print_initial_info(self):
        console.print(
            "[dim]Type your message or use /help for commands. Multi-line: type '...' on empty line.[/]"
        )
        ...
        console.print(
            f"[dim]Streaming mode: {stream_status} (use /stream to toggle)[/]"
        )
        console.print("-" * 60)
        console.print()
    ...
    def _get_multiline_input(self) -> str:
        console.print(
            "[yellow]Multi-line input mode. Type '...' on empty line to finish.[/]"
        )
        lines = []
        while True:
            try:
    ...
    def extract_code_blocks(self, text: str):
        ...
        return [
            (lang or "text", code.strip(), f"```{lang or ''}\n{code.strip()}\n```")
    ...
    def config_menu(self):
        while True:
            ...
            stats_table.add_row(
                "Session Context",
                f"{len(session_memory.get('session_context', ''))} chars",
            )
            stats_table.add_row(
                "Global Profile",
                f"{len(profile_memory.get('player_summary', ''))} chars",
            )
            stats_table.add_row(
                "Global Knowledge", f"{len(global_knowledge.get('facts', ''))} chars"
            )
    ...
    def user_settings_menu(self):
        ...
        if choice == "1":
            new_name = Prompt.ask(
                "Enter your display name", default=profile["display_name"]
            )
            ...
            new_name = Prompt.ask(
                "Enter partner's name", default=profile["partner_name"]
            )
            ...
            try:
                ...
                new_affection = IntPrompt.ask(
                    "Set affection level (0-100)", default=current_affection
                )
        ...
        while True:
            ...
            if keys:
                ...
                for key_name, key_value in keys.items():
                    keys_table.add_row(
                        key_name.upper(), f"{key_value[:8]}...{key_value[-4:]}"
                    )
            ...
            if choice == "1":
                ...
                for i, key_name in enumerate(key_names, 1):
                    console.print(
                        f"{i}. {key_name.upper()}: {keys[key_name][:8]}...{keys[key_name][-4:]}"
                    )
                try:
    ...
    def multimodal_settings_menu(self):
        ...
        if choice == "1":
            ...
            test_prompt = Prompt.ask(
                "Enter test image prompt", default="a cute anime cat with blue eyes"
            )
    ...
    def show_memory_status(self):
        ...
        table.add_row(
            "Session Memory", f"{len(session_memory.get('session_context', ''))} chars"
        )
        table.add_row(
            "Global Profile",
            f"{len(profile.get('memory', {}).get('player_summary', ''))} chars",
        )
        console.print(table)

    def launch_web_interface(
        self, port: int = 5000, host: str = "0.0.0.0", reload: bool = False
    ):
