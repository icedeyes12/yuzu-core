...
def retrieve_static_memories(query=None, limit=15, user_id=None):
    ...
    trgm_results = MemoryDB.search_trgm(
        query=keyword,
        fact_type=FACT_TYPE_STATIC,
        limit=limit,
        user_id=user_id,
    ...
    tsv_results = MemoryDB.search_tsv(
        query=keyword,
        fact_type=FACT_TYPE_STATIC,
        limit=limit,
        user_id=user_id,
...
def retrieve_memories_combined(
    ...
    if not query:
        static = retrieve_static_memories(
            query=None, limit=static_limit, user_id=user_id
        )
        dynamic = retrieve_dynamic_memories(
            session_id, query=None, limit=dynamic_limit, user_id=user_id
        )
    ...
    if query_len < _MIN_QUERY_LEN_FOR_EMBEDDING:
        ...
        static = retrieve_static_memories(
            query=query, limit=static_limit, user_id=user_id
        )
        dynamic = retrieve_dynamic_memories(
    ...
    trgm_results_static = MemoryDB.search_trgm(
        query=keyword,
        fact_type=FACT_TYPE_STATIC,
        limit=static_limit,
        user_id=user_id,
    ...
    trgm_results_dynamic = MemoryDB.search_trgm(
        query=keyword,
        session_id=session_id,
        fact_type=FACT_TYPE_DYNAMIC,
        limit=dynamic_limit,
...
def retrieve_segments(
    session_id: str, query=None, limit: int = 10, user_id: str | None = None
):
    """
    ...
    if query_vec:
        results = MemoryDB.search_similar(
            embedding=query_vec,
            session_id=session_id,
            fact_type=FACT_TYPE_DYNAMIC,
...
async def retrieve_static_memories_async(
    query=None, limit=15, user_id: str | None = None
):
    """
    ...
    # Channel 1: vector (pgvector)
    vec_results = (
        await MemoryDB.search_similar_async(
            embedding=query_vec,
            fact_type=FACT_TYPE_STATIC,
            limit=limit,
            user_id=user_id,
    ...
    # Channel 2: trigram fuzzy (pg_trgm)
    trgm_results = await MemoryDB.search_trgm_async(
        query=keyword,
        fact_type=FACT_TYPE_STATIC,
        limit=limit,
        user_id=user_id,
    ...
    tsv_results = await MemoryDB.search_tsv_async(
        query=keyword,
        fact_type=FACT_TYPE_STATIC,
        limit=limit,
        user_id=user_id,
...
async def retrieve_dynamic_memories_async(
    session_id: str, query=None, limit=10, user_id: str | None = None
):
    """
