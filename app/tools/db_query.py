...
def _validate_query(query: str, write_mode: bool) -> tuple[bool, str]:
    ...
    if _is_write_query(query) and not write_mode:
        return False, f"Write operation detected. Use --write flag: /sql --write {query[:50]}..."
...
def _format_table(rows: list[dict], columns: list[str], max_rows: int = MAX_ROWS) -> str:
...
def _parse_psql_output(output: str, columns: list[str] | None = None) -> tuple[list[dict], list[str]]:
