...
class CustomAnthropicProvider(AIProvider):
            ...
            if response.status_code == 200:
                ...
                for block in content_blocks:
                    if block.get("type") == "text":
                        text += block.get("text", "")
                return text.strip()
            logger.warning("[CustomAnthropic] %s: %s", response.status_code, response.text[:300])
            return None
    ...
    async def send_message_raw(
        ...
        try:
            ...
            logger.warning("[CustomAnthropic] raw %s: %s", response.status_code, response.text[:300])
            return None
        except Exception as e:
    ...
    async def _send_message_streaming_impl(
        ...
        **kwargs,
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                async with client.stream(
                    ...
                    if response.status_code != 200:
                        ...
                        logger.warning("[CustomAnthropic] stream %s: %s", response.status_code, body[:300])
                        yield ""
                        return
            ...
            logger.error("[CustomAnthropic] streaming error: %s", repr(e), exc_info=True)
