...
class CerebrasProvider(AIProvider):
    ...
    def send_message(self, messages: list[dict], model: str, **kwargs) -> str | None:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.debug(
                    f"[Cerebras] Error {response.status_code}: {response.text[:200]}"
                )
            else:
...
class OpenRouterProvider(AIProvider):
    ...
    def send_message(self, messages: list[dict], model: str, **kwargs) -> str | None:
        ...
        try:
            ...
            # Debug: Log summary (not full payload)
            logger.debug(
                f"[OpenRouter] {model} | max_tokens={max_tokens or 'unlimited'}"
            )
    ...
    def send_message_streaming(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"[ERROR] Chutes streaming API error {response.status_code}: {response.text}"
                )
                yield ""
...
class AIProviderManager:
    ...
    def _internal_llm_call(self, messages: list[dict], **kwargs) -> str | None:
        ...
        for attempt in range(3):
            ...
            if attempt < 2:
                logger.debug(
                    f"[INT] chutes/{MAIN_MODEL} failed (attempt {attempt + 1}), retrying..."
                )
                time.sleep(0.5)
        ...
        for attempt in range(2):
            ...
            if result:
                logger.debug(
                    f"[INT] chutes/{FALLBACK_MODEL} OK (attempt {attempt + 1})"
                )
                return result
            ...
            if not _is_connection_error(last_error):
                logger.debug(
                    f"[INT] chutes/{FALLBACK_MODEL} failed (non-retryable error)"
                )
                break

            if attempt < 1:
                logger.debug(
                    f"[INT] chutes/{FALLBACK_MODEL} failed (attempt {attempt + 1}), retrying..."
                )
                time.sleep(0.5)
