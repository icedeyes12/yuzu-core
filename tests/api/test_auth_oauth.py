...
def test_rewrite_redirect_uri_uses_forwarded_public_origin() -> None:
    ...
    assert result == "https://companion.yuzuki.space/api/auth/callback"


def test_rewrite_redirect_uri_keeps_local_origin_without_proxy_headers() -> None:
    request = _request()
    ...
    assert result == "http://localhost:5000/api/auth/callback"
...
def test_login_sets_secure_state_cookie_and_public_redirect(
    ...
    monkeypatch.setenv(
        "OAUTH_GOOGLE_REDIRECT_URI", "http://localhost:5000/api/auth/callback"
    )
    ...
    assert (
        "redirect_uri=https%3A%2F%2Fcompanion.yuzuki.space%2Fapi%2Fauth%2Fcallback"
...
def test_callback_rejects_missing_state_cookie(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SESSION_SECRET", "test-secret")
    request = _request()
    request.scope["path"] = "/api/auth/callback"
...
def test_auth_url_contains_exact_callback() -> None:
    location = "https://accounts.google.com/o/oauth2/v2/auth?redirect_uri=https%3A%2F%2Fcompanion.yuzuki.space%2Fapi%2Fauth%2Fcallback"
    query = parse_qs(urlsplit(location).query)
    assert query["redirect_uri"] == ["https://companion.yuzuki.space/api/auth/callback"]
